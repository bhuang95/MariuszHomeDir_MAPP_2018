...
!uses Barnes analysis approach signle pass only
  ...
  REAL, ALLOCATABLE :: lat(:,:,:), lon(:,:,:),mapscale(:,:,:),&
       &maskout(:,:),dsaindexout(:,:)
  REAL(r_kind), ALLOCATABLE, DIMENSION(:,:) :: maskout_kind(:,:),dsaindexout_kind(:,:)
...
!$omp end parallel
  ...
  ALLOCATE(maskout_kind(nx,ny),dsaindexout_kind(nx,ny))

  nx_kind=nx
  ny_kind=ny
  maskout_kind=maskout
  dsaindexout_kind=dsaindexout

  OPEN(unit=outunit,file=outfile,form='unformatted')
  WRITE(outunit)nx_kind,ny_kind
  WRITE(outunit)maskout_kind
  WRITE(outunit)dsaindexout_kind
  CLOSE(outunit)

  DEALLOCATE(lat,lon,mapscale,maskout,dsaindexout,maskout_kind,dsaindexout_kind)
