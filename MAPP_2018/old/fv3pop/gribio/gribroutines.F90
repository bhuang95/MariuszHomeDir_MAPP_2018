...
!********************************************************************
      SUBROUTINE writegrib(var_name,nx,ny,nz,glvl,ct,data,date, nt_h, tba, vlevels)
        USE grib_datastru
        IMPLICIT NONE

        CHARACTER*(*), INTENT(IN):: var_name, date
        INTEGER, INTENT(IN) :: nx, ny, nz, nt_h, glvl, ct
        INTEGER,INTENT(IN)::tba
        REAL, INTENT(IN):: data(*), vlevels(*)
...
! declare variables for grib table entries
        ...
        PRINT*, 'glvl, ct =', glvl, ct
        PRINT*, 'data(nx*ny*nz-100:nx*ny*nz) =', data(nx*ny*nz-100:nx*ny*nz)
        PRINT*, 'date =', trim(date)
        PRINT*, 'nt_h, tba', nt_h, tba
#endif
...
! which in turn called by w3fi72.
        id(1) =  28        ! number of bytes in PDS
        id(2) =  141         ! parm_table version
        id(3) =  7        ! id_center:  GSD = 59, NCEP = 7
        ...
        id(9) = 1 !ztype(idx) ! indicator of level type.
        id(10) = 0 !iz1(idx)  ! value 1 of level
        id(11) = 0 !iz2(idx)  ! value 2 of level
        ...
        tu = 1
        nt = nt_h
        IF (nt > 255) THEN  ! a temp kluge to take care of > 256 hour problem
          nt = nt_h / 24
          tu = 2            ! day
          IF (nt > 255) THEN 
            nt = nt_h / (7 * 24)
        ...
        id(17) = tu         ! forecast time unit: 0 minute, 1 hour, 2 day, 3 month, 4 year,... 8 century, and 253 week (new definitio).
...
! if data is icosahedral model grid      
        IF (ny == 1) THEN
          ...
          gflag = 1
        ENDIF
        IF (nz == 1 .AND. ztype(idx) == 100) THEN
          READ(var_name(2:), *, iostat=stat) pv
          IF (stat /= 0) THEN
            PRINT*, 'String conversion to integer failed!'
            STOP
          ENDIF
          CALL create_levels(ztype(idx),nz, levels, pv) 
        ELSE IF (nz == 63 .OR. nz == 64) THEN
          levels(1:nz) = INT(vlevels(1:nz)*1.0) 
        ELSE
...
!        write(6,*) 'opengrib: opened gribfile ', trim(gribfile), &
!                   ' to grib_lun=', grib_lun
...
! closes an grib file using 'C' style close statement
      ...
      SUBROUTINE create_levels(ztype, nz, levels, mb) 
        IMPLICIT NONE
        INTEGER ztype, nz, levels(*)
        INTEGER, OPTIONAL, INTENT(IN) :: mb
        
        INTEGER n_std_pres_levels, n_reg_pres_levels
        PARAMETER(n_std_pres_levels=17) 
        PARAMETER(n_reg_pres_levels=37) 
        INTEGER i
