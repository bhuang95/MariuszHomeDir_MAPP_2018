...
C--------------------------------------------------------------------
C
c C$            1.     CREATE WMO HEADER.
C
c C$            1.1    CONVERT BULHED FROM EBCDIC TO ASCII.
C
...
C
c C$            1.2    PICK OFF THE DAY OF MONTH (YY)
c C$                   AND CONVERT TO ASCII.
C
      write(ctemp,fmt='(I2.2)') IDAY
      WMOHDR(13)=ctemp(1:1)
      WMOHDR(14)=ctemp(2:2)
C
c C$            1.3    PICK OFF THE HOUR(GG) AND CONVERT TO ASCII.
C
...
C--------------------------------------------------------------------
C
c C$            2.     MOVE WMOHDR TO OUTPUT FIELD.
C
      DO 200 I = 1,21
        HEADER(I) = WMOHDR(I)
  200 CONTINUE
