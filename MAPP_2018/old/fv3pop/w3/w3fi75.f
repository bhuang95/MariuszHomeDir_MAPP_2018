     ...
     &  NPTS,BDS11,IPFLD,PFLD,LEN,LENBDS,IBERR,PDS,IGDS) !JFM
...
C               - 2, W3FI58 DYNAMIC RANGE OVERFLOW.
C
...
C
      INTEGER         IWORK(NPTS)
      integer         LENBDS       !JFM
      integer         LENBDSA(1)   !JFM
      integer         nfill  (1)   !JFM
      integer         iscalea(1)   !JFM
      integer         iexpa  (1)   !JFM
      integer         imanta (1)   !JFM
      integer         nbitsa (1)   !JFM
C
      LOGICAL         CONST
C
...
C
              CALL W3FI58(IWORK,NPTS,IWORK,PFLD,NBITS,LEN,KMIN,IERR)
              IF (IERR == 1) THEN
                 IBERR = 2
                 RETURN
              ENDIF
...
C                               BYTES 1-3
          LENBDSA(1) = LENBDS
          CALL SBYTEC (BDS11,LENBDSA(1),0,24) !JFM
C
C                               BYTE  4
C                                       FLAGS
...
C                               BYTE  5-6
          IF (ISCALE.LT.0) THEN
              ...
              ISCALE  = - ISCALE
              iscalea = iscale                  !JFM
              CALL SBYTEC (BDS11,ISCALEa,33,15) !JFM
          ELSE
              iscalea = iscale                  !JFM
              CALL SBYTEC (BDS11,ISCALEa,32,16) !JFM
          END IF
C
...
C          CALL W3FI01(LW)
          ...
          iexpa (1) = iexp                   !JFM
          imanta(1) = IMANT                  !JFM
          CALL SBYTEC (BDS11,IEXPa ,48, 8)   !JFM
          CALL SBYTEC (BDS11,IMANTa,56,24)   !JFM
C
C
...
C                               BYTE  11
          nbitsa(1) = nbits               !JFM
          CALL SBYTEC (BDS11,NBITSa,80,8) !JFM
      END IF
C
      RETURN
      END
