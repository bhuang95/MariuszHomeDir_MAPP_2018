...
C                  9, W3FI75 (W3FI58) ERROR, PACK ROUTINE DYNAMIC RANGE OVERFLOW. 
C
C   OUTPUT FILES:
...
C             NEED EVEN NUMBER OF OCTETS
              NLEFT  = 16 - MOD(IBLEN,16)
              ALLOCATE(BMS(NPTS/8+2+6))
          ELSE
              NLEFT  = 0
...
C
C Commented out the if test to prevent debugging (-C) to crash on the call to W3FI75 - JFM
CJFM  IF(IBDSFL(2).NE.0) THEN
        ALLOCATE(IPFLD(NPTS*4))
        IPFLD=char(0)
CJFM  ENDIF
cJFM  Initialized PFLD because it's passed to w3fi75.f to w3fi58.f to sbytes.f where it is used.
      pfld = char(0)
C
...
c     JFM added MovChar so Lahey would not see arbitrary values in PFLD
      call MovChar(KBUF(ISTART+1),BDS11,11)
c     do ii = 1, 11
c        KBUF(ISTART+ii) = BDS11(ii)
c     enddo
C
...
c       JFM added MovChar so Lahey would not see arbitrary values in PFLD
        call MovChar(KBUF(ISTART+1),PFLD,LEN)
c       do ii = 1, LEN
c         KBUF(ISTART+ii) = PFLD(ii)
c       enddo
      END IF
C
