...
!                       close_cdf,  write_cdf_cdfapi
      ...
      NAMELIST  /fv3post/ yyyymmddhhmmss, cres, nz, inputdir, gridfiledir, outputdir, input_filetype, &
...
! if interpolation scheme is not 0, initialize the horizontal interpolation.
      ...
      IF (output_fmt.EQ."grib") THEN
         gribfile = TRIM(outputdir) // '/' // 'aod_'// yyyymmddhhmmss(1:10)//".grib"
         PRINT *,TRIM(gribfile)
         CALL opengrib(gribfile)
      ELSEIF (output_fmt.EQ."bin") THEN
         binfile = TRIM(outputdir) // '/' // jdate(1:LEN_TRIM(jdate)) //  '.LL'
        ...
        DO nt = t1, t2, delta_t
           ...
           DO i = 1, 6
              ...
              IF (input_filetype == 'aod') THEN
                 data_file = TRIM(inputdir) // "/aod_"//yyyymmddhhmmss(1:10) //  ".tile" // ich//"_"//ct//".nc"
              ELSE
          ...
          DO var_num = 1, numvars ! for each variable
            ...
            IF (.NOT. isVvar(var_name)) THEN
              CALL read_1var(ncids, 1, var_name, vardata, nlevels, vlevels)
            ENDIF
