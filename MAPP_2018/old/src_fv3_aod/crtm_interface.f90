MODULE crtm_interface
  ...
  USE module_fv3_gocart, ONLY: naero_gocart_fv3, n_aerosols_crtm, aeronames_gocart_fv3_gsi
...
! Get indexes of variables composing the jacobian_aero
    ...
    IF(n_aerosols>0)THEN
       ...
       IF (n_aerosols == naero_gocart_fv3) THEN
          n_aerosols_jac=n_aerosols_crtm+1
          logwrf_gocart=.TRUE.
          aero_names=aeronames_gocart_fv3_gsi
       ...
       ELSE
          ...
          DO i=1,n_aerosols_crtm
             IF (aeronames_gocart_fv3_gsi(i) /= 'p25' ) THEN
                j=j+1
                aero_names(j)=aeronames_gocart_fv3_gsi(i)
             ENDIF
          ENDDO
       ENDIF
    ELSE
