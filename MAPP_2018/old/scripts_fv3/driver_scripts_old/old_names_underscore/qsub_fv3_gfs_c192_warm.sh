...
#qsub -v ident=2015080100 qsub_fv3_gfs_c192_warm.sh
...
BACK_DATEDIR=${FV3_DATA}/${resolution}/${backdate}

if [[ $ident == $start_ident ]]
then
    start='T'
    GFS_INDIR=${DATEDIR}/INPUT
    TRACER_INDIR=${DATEDIR}/INPUT/tracers
else
    start='F'
    GFS_INDIR=${DATEDIR}/RESTART_cold
    TRACER_INDIR=${BACK_DATEDIR}/RESTART
fi
...
clim=${FV3_CLIM}
OUTDIR=${DATEDIR}/OUTPUT
WORKDIR=${FV3_BASEDIR}/workdir_${resolution}_warm
...
/bin/cp ${SCRIPTDIR}/diag_table_warm diag_table
...
then
	...
	do
	    ctile=tile${itile}
	    ncks -A ${GFS_INDIR}/gfs_tracer.res.${ctile}.nc ${TRACER_INDIR}/fv_tracer.res.${ctile}.nc
    ...
    do
	ctile=tile${itile}
	ncks -A ${GFS_INDIR}/gfs_tracer.res.${ctile}.nc ${TRACER_INDIR}/${year}${month}${day}.${hour}0000.fv_tracer.res.${ctile}.nc
...
/bin/rm -rf ${DATEDIR}/RESTART

#mkdir -p ${DATEDIR}/RESTART
#/bin/mv RESTART/*fv_tracer.res.* ${DATEDIR}/RESTART

/bin/mv RESTART ${DATEDIR}
...
then
    cd $SCRIPTDIR
    echo "qsub -v ident=$ident qsub_fv3_gfs_c192_warm.sh"
    qsub -v ident=$ident qsub_fv3_gfs_c192_warm.sh
    exit 0
else
