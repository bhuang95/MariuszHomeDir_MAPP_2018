...
WORKDIR=${FV3_BASEDIR}/workdir_${resolution}
...
EMISS_BIOBURNDIR=${FV3_DATA}/${resolution}/${analdate}/emiss_bioburn_data
...
year=`echo ${analdate} | cut -c1-4`
mon=`echo ${analdate} | cut -c5-6`
day=`echo ${analdate} | cut -c7-8`
hour=`echo ${analdate} | cut -c10-11`
...
# Date specific ICs
ln -s ${INDIR}/*.nc ./INPUT
ln -s ${INDIR}/tracers/fv_tracer.res.tile?.nc ./INPUT

#ln -s ${INDIR}/tracers/tmp/fv_tracer.res.tile?.nc ./INPUT
#/bin/cp ${INDIR}/tracers/fv_tracer.res.tile?.nc ./INPUT
...
#bioburn will be daily files at some point
    ...
    ln -s ${EMISS_BIOBURNDIR}/emiss_bioburn.${ctile}_${analdate}.nc ./INPUT/emiss_bioburn.${ctile}.nc
