...
#qsub -v ident=2015080100,member=member qsub_fv3_gfs_c192_warm_da_ensemble.sh
#e.g. qsub -v ident=2015080100,member=ensmean qsub_fv3_gfs_c192_warm_da_ensemble.sh
#e.g. qsub -v ident=2015080100,member=1 qsub_fv3_gfs_c192_warm_da_ensemble.sh
...
clim=${FV3_CLIM}
OUTDIR=${FV3_RUNS}/${sim}/${analdate}/${charnanal}/OUTPUT_FV3
...
if [[ $? != 0 ]]
then
    echo "FV3 crashed for member $charnanal"
    exit 1
fi
...
cd $SCRIPTDIR_UTIL
echo "qsub -v ident=$ident,member=$member qsub_run_regrid-nemsio_4da_ensemble.sh"
qsub -v ident=$ident,member=$member qsub_run_regrid-nemsio_4da_ensemble.sh

echo "qsub -v ident=$ident,member=$member qsub_calc_aod_ensemble.sh"
qsub -v ident=$ident,member=$member qsub_calc_aod_ensemble.sh
