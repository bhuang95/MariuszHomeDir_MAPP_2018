#!/bin/ksh
#SBATCH -J gsi_aod_nnr
#SBATCH -A wrf-chem
#SBATCH -n 96
#SBATCH -t 00:30:00
#SBATCH -q debug
#SBATCH -D /home/Mariusz.Pagowski/codes/scripts_fv3/da_scripts
#SBATCH -o /scratch3/BMC/chem-var/pagowski/tmp/FV3GFS/qslogs/%x.o%j
#SBATCH -e /scratch3/BMC/chem-var/pagowski/tmp/FV3GFS/qslogs/%x.e%j

#sbatch --export=ALL,ident=2015080500 sbatch_gsi_fv3_aod_nnr_4iugg.ksh


analdate_start=2015080500
analdate_end=2015081618

charnanal='ensmean'

GSIPROC=$SLURM_NTASKS
...
# GSI_EXE  = path and name of the gsi executable 
  ...
  WORK_ROOT=${MAINDIR}/tmp_dirs/workdir_gsi_ensemble
#TBD
  ...
  BK_ROOT=${FV3_RUNS}/${sim}/${analdatem}/${charnanal}/OUTPUT_FV3_nemsio

  CRTM_ROOT=${GSI_ROOT}/crtm_coeffs
  FIX_ROOT=${GSI_ROOT}/fix
  GSI_EXE=${GSI_EXEC}

  GSI_NAMELIST=${SCRIPTDIR_DA}/namelist_gsi_fv3_aod_nnr_4iugg.sh
...
RUN_COMMAND="srun -n $SLURM_NTASKS"
...
GSI_EXECDIR=${GSI_BASEDIR}/src_4iugg
...
if [ -s ./sfcf03 ] && [ -s ./sfcf06 ] && [ -s ./sfcf09 ] && [ -s ./sigf03 ] && [ -s ./sigf06 ] && [ -s ./sigf09 ] ; then
    cat gsiparm.anl
    echo "${RUN_COMMAND} ${GSI_EXEC} > ${LOG_DATE_DIR}/gsi.out 2>&1"
    ${RUN_COMMAND} $GSI_EXEC > ${LOG_DATE_DIR}/gsi.${charnanal}.4iugg.out 2>&1
    rc=$?
    if [[ $rc -ne 0 ]];then
    ...
    OUTDIR=${FV3_RUNS}/${sim}/${analdate}/${charnanal}/OUTPUT_GSI_nemsio
    ...
    /bin/cp ${LOG_DATE_DIR}/gsi.${charnanal}.4iugg.out ${OUTDIR}

#    tar -cf obs_input.tar obs_input* 

#    /bin/mv obs_input.tar ${OUTDIR}

    cat pe????.modis_aod_aqua_01 > ${OUTDIR}/diag_nnr_ges.${analdate}_${charnanal}.4iugg

else
    echo "Input files missing for $charnanal"
    exit 1
fi
...
if [[ $ident -le $analdate_end ]]
then
    echo "sbatch --export=ALL,ident=$ident sbatch_gsi_fv3_aod_nnr_4iugg.ksh"
    sbatch --export=ALL,ident=$ident sbatch_gsi_fv3_aod_nnr_4iugg.ksh
fi
