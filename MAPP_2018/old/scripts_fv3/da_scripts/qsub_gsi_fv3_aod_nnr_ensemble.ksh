...
#qsub -v ident=2015080106 qsub_gsi_fv3_aod_nnr_ensemble.ksh
...
if [ -s ./sfcf03 ] && [ -s ./sfcf06 ] && [ -s ./sfcf09 ] && [ -s ./sigf03 ] && [ -s ./sigf06 ] && [ -s ./sigf09 ] ; then
    ...
    ${RUN_COMMAND} $GSI_EXEC > ${LOG_DATE_DIR}/gsi.${charnanal}.out 2>&1
    rc=$?
    if [[ $rc -ne 0 ]];then
    ...
    /bin/cp ${LOG_DATE_DIR}/gsi.${charnanal}.out ${OUTDIR}

    tar -cf obs_input.tar obs_input* 

    /bin/mv obs_input.tar ${OUTDIR}

    cat pe????.modis_aod_aqua_01 > ${OUTDIR}/diag_nnr_ges.${analdate}_${charnanal}
...
do
    ...
    echo "GSI OBSERVER running $charnanal"
...
EOF
	...
	$nln ${GFS_DIR_ENS}/sfcanl_${analdate}_${charnanal} ./sfcanl
    fi
...
echo "qsub -v ident=${ident} qsub_enkf_fv3_aod_nnr.ksh"

qsub -v ident=${ident} qsub_enkf_fv3_aod_nnr.ksh
