...
# GSI_EXE  = path and name of the gsi executable 
  ...
  WORK_ROOT=${MAINDIR}/tmp_dirs/workdir_gsi_hybrid
#TBD
  ...
  OBS_ROOT=$OBS_DIR
  VIIRSBUFR=${OBS_ROOT}/viirs_hdf/VIIRS_AOD_BUFR:${analday}${analctime}

  analdatem=`$ndate -$ANALINC $analdate`

  BK_ROOT=${FV3_RUNS}/${sim}/${analdatem}/OUTPUT_FV3_nemsio
...
if [ ${bkcv_option} = GLOBAL ] ; then
  ...
  ANAVINFO=${FIX_ROOT}/anavinfo_fv3_gocart_hybrid
...
if [ ${bk_core} = GLOBAL ] ; then
	...
	then
		...
		$nln ${BK_ROOT}/${charnanal}/sfg_${analdate}_fhr${cfh}_${charnanal} ${ensemble_path}sigf${cfh}_ens_${charnanal}
...
if [ -s ./sfcf03 ] && [ -s ./sfcf06 ] && [ -s ./sfcf09 ] && [ -s ./sigf03 ] && [ -s ./sigf06 ] && [ -s ./sigf09 ] ; then
    ...
    OUTDIR=${FV3_RUNS}/${sim}/${analdate}/OUTPUT_GSI_nemsio
...
qsub -v ident=${ident} qsub_calc_increment_nemsio.sh
