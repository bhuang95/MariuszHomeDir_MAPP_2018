...
#qsub -v ident=2015080106 qsub_gsi_fv3_aod_nnr.ksh
...
if [ -s ./sfcf03 ] && [ -s ./sfcf06 ] && [ -s ./sfcf09 ] && [ -s ./sigf03 ] && [ -s ./sigf06 ] && [ -s ./sigf09 ] ; then
    ...
    cfh=06
    $nln ${BK_ROOT}/sfg_${analdate}_fhr${cfh} ./file_fcst
    $nln siganl ./file_anal

    file_increment=increment_${analdate}.nc
    /bin/cp ${FV3_FIXDIR}/C${RES}/fv3_increment_zero.nc $file_increment

    ${EXEC} file_fcst file_anal $file_increment
    if [[ $? != 0 ]]
    then
	echo "calc_increment_nemsio.x crashed"
	exit 1
    fi
    ...
    $nln $file_increment fv3_increment.nc

    /bin/mv $file_increment fv3_increment.nc $OUTDIR 
