...
#qsub -v ident=2015080106 qsub_gsi_fv3_aod_viirs.ksh
...
if [ -s ./sfcf03 ] && [ -s ./sfcf06 ] && [ -s ./sfcf09 ] && [ -s ./sigf03 ] && [ -s ./sigf06 ] && [ -s ./sigf09 ] ; then
    ...
    ${RUN_COMMAND} $GSI_EXEC > ${LOG_DATE_DIR}/gsi.out 2>&1
    rc=$?
    if [[ $rc -ne 0 ]];then
    ...
    /bin/cp ${LOG_DATE_DIR}/gsi.out ${OUTDIR}
...
echo "qsub -v ident=${ident} qsub_calc_increment_nemsio.sh"
