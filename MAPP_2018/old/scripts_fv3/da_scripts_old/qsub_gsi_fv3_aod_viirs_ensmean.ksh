...
#qsub -v ident=2015081012 qsub_gsi_fv3_aod_viirs_ensmean.ksh
...
# GSI_EXE  = path and name of the gsi executable 
  ...
  WORK_ROOT=${BASEDIR}/tmp_dirs/workdir_gsi
#TBD
  ...
  BK_ROOT=${FV3_RUNS}/DA_ENKF/${analdatem}/OUTPUT_FV3_nemsio/ensmean
...
if [ -s ./sfcf03 ] && [ -s ./sfcf06 ] && [ -s ./sfcf09 ] && [ -s ./sigf03 ] && [ -s ./sigf06 ] && [ -s ./sigf09 ] ; then
    ...
    OUTDIR=${FV3_RUNS}/DA_ENKF/${analdate}/OUTPUT_GSI_nemsio/ensmean
    ...
    tar -cf obs_input.tar obs_input* *.bin *bufr satbias_in satbias_angle

    /bin/mv obs_input.tar ${OUTDIR}

    cat pe????.viirs_aod_viirs_01 > ${OUTDIR}/diag_viirs_ges.${analdate}_ensmean
