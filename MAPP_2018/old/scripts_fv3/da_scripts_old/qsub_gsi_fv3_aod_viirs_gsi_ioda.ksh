...
#PBS -A chem-var
#PBS -N gsi_aod_viirs_ioda
#PBS -q debug
##PBS -q urgent
#PBS -d /home/Mariusz.Pagowski/codes/scripts_fv3/da_scripts_ioda
#PBS -o /scratch3/BMC/chem-var/pagowski/tmp/FV3GFS/qslogs
#PBS -e /scratch3/BMC/chem-var/pagowski/tmp/FV3GFS/qslogs

#qsub -v ident=2015081000 qsub_gsi_fv3_aod_viirs_gsi_ioda.ksh
...
# GSI_EXE  = path and name of the gsi executable 
  ...
  WORK_ROOT=${BASEDIR}/tmp_dirs/workdir_ioda
#TBD

  analday=`echo ${analdate} | cut -c1-8`
  analtime=`echo ${analdate} | cut -c9-10`

  if [[ $analtime -gt 21 && $analtime -le 3 ]]
  then
      analtime=6
  fi

  if [[ $analtime -gt 3 && $analtime -le 9 ]]
  then
      analtime=6
  fi

  if [[ $analtime -gt 9 && $analtime -le 15 ]]
  then
      analtime=12
  fi

  if [[ $analtime -gt 15 && $analtime -le 21 ]]
  then
      analtime=18
  fi
  ...
  OBS_ROOT=${OBS_DIR}/viirs_bufr_concat_sampled
  VIIRSBUFR=${OBS_ROOT}/VIIRS_AOD_BUFR:${analday}${analctime}

  analdatem=`$ndate -$ANALINC $analdate`

#  RUN_ROOT=/scratch3/BMC/wrf-chem/pagowski/tmp/fv3GFS_old/FV3_RUNS/DA_GSI_test_003

#  RUN_ROOT=/scratch3/BMC/wrf-chem/pagowski/tmp/FV3GFS/FV3_RUNS/DA_ENKF
  RUN_ROOT=/scratch3/BMC/chem-var/pagowski/tmp/FV3GFS/FV3_RUNS/DA_ENKF

  BK_ROOT=${RUN_ROOT}/${analdatem}/ensmean/OUTPUT_FV3_nemsio

  CRTM_ROOT=${GSI_ROOT}/crtm_coeffs
  FIX_ROOT=${GSI_ROOT}/fix
  GSI_EXECDIR=${GSI_ROOT}/src_ioda
  GSI_EXE=${GSI_EXEC}
  GSI_NAMELIST=${SCRIPTDIR_DA}/namelist_gsi_fv3_aod_viirs_observer.sh
...
if [ ${bkcv_option} = GLOBAL ] ; then
  ...
  ANAVINFO=${FIX_ROOT}/anavinfo_fv3_gocart_nop25
...
if [ ${bk_core} = GLOBAL ] ; then
    ...
    $nln ${GFS_DIR}/sfcanl_${analdate} ./sfcanl
fi

if [ -s ./sfcf03 ] && [ -s ./sfcf06 ] && [ -s ./sfcf09 ] && [ -s ./sigf03 ] && [ -s ./sigf06 ] && [ -s ./sigf09 ] ; then
    cat gsiparm.anl
    echo "${RUN_COMMAND} ${GSI_EXEC} > gsi_ioda.out 2>&1"
    ${RUN_COMMAND} $GSI_EXEC > gsi_ioda.out 2>&1
    rc=$?
    if [[ $rc -ne 0 ]];then
    ...
    OUTDIR=${RUN_ROOT}/${analdate}/${charnanal}/OUTPUT_FV3_nemsio_ioda
    ...
    cat pe????.viirs_aod_suomi_npp_01 > ${OUTDIR}/aod_viirs_diag_${analdate}.bin

    cat pe????.viirs_aod_suomi_npp_vars_01 > ${OUTDIR}/aod_viirs_geoval_${analdate}.bin

    mpirun -np 2 /scratch3/BMC/chem-var/pagowski/jedi_fv3/gsi.fd/build/bin/nc_diag_cat.x -o aod_diag_geoval.nc4 *nc4

    ncatted -O -a date_time,global,m,i,2018041500 aod_diag_geoval.nc4
#    ncatted -O -a date_time,global,m,i,2016100400 aod_diag_geoval.nc4

    /bin/mv aod_diag_geoval.nc4 ${OUTDIR}
