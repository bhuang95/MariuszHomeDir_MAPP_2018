...
#qsub -v ident=2015081000 qsub_gsi_fv3_aod_nnr_gsi_ioda.ksh
...
#  RUN_ROOT=/scratch3/BMC/wrf-chem/pagowski/tmp/FV3GFS/FV3_RUNS/DA_ENKF
  ...
  GSI_EXE=${GSI_EXEC}
  GSI_NAMELIST=${SCRIPTDIR_DA}/namelist_gsi_fv3_aod_nnr_observer.sh
...
if [ -s ./sfcf03 ] && [ -s ./sfcf06 ] && [ -s ./sfcf09 ] && [ -s ./sigf03 ] && [ -s ./sigf06 ] && [ -s ./sigf09 ] ; then
    ...
    OUTDIR=${RUN_ROOT}/${analdate}/ensmean/OUTPUT_FV3_nemsio_ioda
    ...
    cat pe????.modis_aod_aqua_01 > ${OUTDIR}/diag_nnr_ges.${analdate}.bin

    cat pe????.modis_aod_aqua_vars_01 > ${OUTDIR}/vars_nnr_ges.${analdate}.bin
