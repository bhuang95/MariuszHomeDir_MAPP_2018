...
charnanal_ensmean=control
charnanal_ensmean_sfc=ensmean
...
# GSI_EXE  = path and name of the gsi executable 
  ...
  WORK_ROOT=${BASEDIR}/workdir_gsi_hybrid
#TBD
  ...
  BK_ROOT=${FV3_RUNS}/DA_ENKF/${analdatem}/OUTPUT_FV3_nemsio

  CRTM_ROOT=${GSI_ROOT}/crtm_coeffs
  FIX_ROOT=${GSI_ROOT}/fix
  GSI_EXE=${GSI_EXEC}
  GSI_NAMELIST=${SCRIPTDIR_DA}/namelist_gsi_fv3_aod_viirs_hybrid.sh
#@mzp
 # GSI_NAMELIST=${SCRIPTDIR_DA}/namelist_gsi_fv3_aod_viirs.sh
...
if [ ${bk_core} = GLOBAL ] ; then
	...
	$nln ${BK_ROOT}/${charnanal_ensmean_sfc}/bfg_${analdate}_fhr${cfh}_${charnanal_ensmean} ./sfcf${cfh}
	$nln ${BK_ROOT}/${charnanal_ensmean_sfc}/sfg_${analdate}_fhr${cfh}_${charnanal_ensmean} ./sigf${cfh}

	nanal=1
	while [[ $nanal -le $nanals ]]
	do
	    charnanal=mem`printf %03i $nanal`
	    $nln ${BK_ROOT}/${charnanal}/bfg_${analdate}_fhr${cfh}_${charnanal} .

	    $nln ${BK_ROOT}/${charnanal}/sfg_${analdate}_fhr${cfh}_${charnanal} ./sigf${cfh}_ens_${charnanal}

	    $nln ${GFS_DIR}/sfcanl_${analdate}_${charnanal} ./sfcanl_${charnanal}
    ...
    $nln ${GFS_DIR}/sfcanl_${analdate}_${charnanal_ensmean_sfc} ./sfcanl
fi

if [ -s ./sfcf03 ] && [ -s ./sfcf06 ] && [ -s ./sfcf09 ] && [ -s ./sigf03 ] && [ -s ./sigf06 ] && [ -s ./sigf09 ] ; then
    cat gsiparm.anl
    echo "${RUN_COMMAND} ${GSI_EXEC} > ${LOG_DATE_DIR}/gsi_${charnanal_ensmean}.out 2>&1"
    ${RUN_COMMAND} $GSI_EXEC > ${LOG_DATE_DIR}/gsi_${charnanal_ensmean}.out 2>&1
    rc=$?
    if [[ $rc -ne 0 ]];then
    ...
    OUTDIR=${FV3_RUNS}/DA_ENKF/${analdate}/OUTPUT_HYBRID_nemsio
    ...
    /bin/mv siganl ${OUTDIR}/siganl_${analdate}_control
