...
#PBS -A chem-var
#PBS -N gsi_aod_viirs_ens
##PBS -q debug
#PBS -q urgent
#PBS -d /home/Mariusz.Pagowski/codes/scripts_fv3/da_scripts_old
#PBS -o /scratch3/BMC/chem-var/pagowski/tmp/fv3GFS_old/qslogs
#PBS -e /scratch3/BMC/chem-var/pagowski/tmp/fv3GFS_old/qslogs

#qsub -v ident=2015081012 qsub_gsi_fv3_aod_viirs_ensemble.ksh
...
do
  ...
  BK_ROOT=${FV3_RUNS}/DA_ENKF/${analdatem}/OUTPUT_FV3_nemsio/${charnanal}

  if [[ ! -r $LOG_DATE_DIR ]]
  then
      LOG_DATE_DIR=${LOGDIR}/${analdate}/${charnanal}
...
if [ ${bk_core} = GLOBAL ] ; then
    ...
    do
	cfh=`printf %02i $fh`
	$nln ${BK_ROOT}/bfg_${analdate}_fhr${cfh}_${charnanal} ./sfcf${cfh}
	$nln ${BK_ROOT}/sfg_${analdate}_fhr${cfh}_${charnanal} ./sigf${cfh}
	((fh=fh+1))
    done
    
    $nln ${GFS_DIR}/sfcanl_${analdate}_${charnanal_sfc} ./sfcanl
fi
...
if [ -s ./sfcf03 ] && [ -s ./sfcf06 ] && [ -s ./sfcf09 ] && [ -s ./sigf03 ] && [ -s ./sigf06 ] && [ -s ./sigf09 ] ; then
    ...
    tar -xvf ${FV3_RUNS}/DA_ENKF/${analdate}/OUTPUT_GSI_nemsio/ensmean/obs_input.tar

    echo "${RUN_COMMAND} ${GSI_EXEC} > ${LOG_DATE_DIR}/gsi_${charnanal}.out 2>&1"
    ${RUN_COMMAND} $GSI_EXEC > ${LOG_DATE_DIR}/gsi_${charnanal}.out 2>&1
    rc=$?
    if [[ $rc -ne 0 ]];then
    ...
    OUTDIR=${FV3_RUNS}/DA_ENKF/${analdate}/OUTPUT_GSI_nemsio/${charnanal}
    ...
    cat pe????.viirs_aod_viirs_01 > ${OUTDIR}/diag_viirs_ges.${analdate}_${charnanal}
