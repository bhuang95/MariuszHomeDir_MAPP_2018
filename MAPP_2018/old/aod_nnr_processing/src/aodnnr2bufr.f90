...
!  MP, March 2014
  ...
  INTEGER :: nbatches,batchlen,jday_start,jday_current,jday_current_tmp,hour_current,i,j,k
...
!ncdf file names
  ...
  IF ( (ocean .OR. land .or. deep) == .FALSE. ) THEN
     PRINT *,'Wrong names ocean/land/deep ',ocean,land,deep
  ENDIF

  IF ( (ocean .AND. land .AND. deep) == .TRUE. ) THEN
     PRINT *,'Wrong names ocean/land/deep ',ocean,land,deep
  ENDIF
  ...
  DO i=1,nbatches
     DO j=1,batchlen  
        IF (qc(i,j) == 0) THEN
           ...
           IF (time(i,j) < 0.) THEN
              jday_current_tmp=jday_current-1
              hour=(time(i,j)+1440)/60
              minute=MOD(time(i,j)+1440,60)
           ELSE
              jday_current_tmp=jday_current
              hour=time(i,j)/60
              minute=MOD(time(i,j),60)
           ENDIF

           CALL gdate (jday_current_tmp, year,month,day)
