...
!to convert VIIRS aot and dust/smoke mask hdf files to bufr
  ...
  REAL, PARAMETER :: aotmax=5.,unknown=-999.,sfc_fill=255
  INTEGER, PARAMETER :: nchannels=11
  REAL, PARAMETER, DIMENSION(nchannels) :: channels=(/&
       &0.412e-6,0.445e-6,0.488e-6,0.555e-6,0.672e-6,0.746e-6,&
       &0.865e-6,1.240e-6,1.378e-6,1.610e-6,2.250e-6/)
!do not output channel 9 at 1.378e-6 um
...
!adp - maskfile
  ...
  INTEGER :: rank_adp_flags,rank_adp_qflags,rank_lonm,rank_latm
...
!begin bufr
  ...
  CHARACTER(len=8), PARAMETER :: subset='NC008042'
...
!write viirs bufr  
  ...
  OPEN(unit_table,file='AOT_BUFR_Table_old.txt')
  ...
  DO i=1,dims_lat(1)
     DO j=1,dims_lat(2)
        IF (qaot(i,j,1) > 0) THEN
           ...
           hdr_2(1,1:8)=channels(1:8)
           hdr_2(1,9:10)=channels(10:11)
           hdr_2(1,11)=0.550e-6
           CALL ufbrep(unit_bufr,hdr_2,1,nchannels,iret,hdstr_2)
           hdr_3(1,1:8)=aots(i,j,1:8)
           hdr_3(1,9:10)=aots(i,j,10:11)
           hdr_3(1,11)=aot550(i,j)
...
!eliminate volcanic ash, cloud, unclear, snow/ice
           ...
           IF ( aot550(i,j) < 0 .OR. qaot(i,j,1) < 3) CYCLE
...
!write out dust mask file
              WRITE(unit_dust_mask,'(2f13.5,a15,i3)')&
...
!write out smoke mask file
              WRITE(unit_smoke_mask,'(2f13.5,a15,i3)')&
                   &latm(i,j),lonm(i,j),cidate,&
