...
!convert nc smoke obtained from grib with ncl_convert2nc to regular nc smoke
  ...
  CHARACTER(len=20), DIMENSION(2) :: dimstring_time=&
       &(/"DateStrLen","Time"/)
  ...
  INTEGER, DIMENSION(2) :: end_dims_time, start_dims_time=(/1,1/)
  INTEGER :: timeid
  ...
  end_dims_time(1)=date_strlen
  end_dims_time(2)=end_dims(4)
...
!that produces to small mu in from grib file so that compensates a little
  ...
  status = nf_put_vara_text(mcid,timeid,start_dims_time,end_dims_time,wrftimes)
