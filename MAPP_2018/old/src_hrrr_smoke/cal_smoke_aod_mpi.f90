...
!Mariusz Pagowski Nov, 2016
  ...
  INTEGER(i_kind) :: nchan_viirs=11,nchan_modis=20,nchan_abi=6
  ...
  INTEGER, PARAMETER :: naero_smoke_wrf=3

  CHARACTER(len=max_varname_length), DIMENSION(naero_smoke_wrf), &
       &PARAMETER :: &
       aeronames_smoke_wrf=(/'BC1','OC1','P25'/)

  CHARACTER(len=max_varname_length), PARAMETER :: &
       aeronames_smoke_tracer='tracer_1a'

  REAL :: weight_bc1=0.79,weight_oc1=0.17,weight_p25=0.04
...
! nproc is process number, numprocs is total number of processes.
  ...
  IF (iargc() < 3) THEN
     WRITE(6,*)'Input/Output file names and obstype required - stopping'
...
!remember that genqsat is different for a global model - recheck with the current version
  ...
  varname=TRIM(aeronames_smoke_tracer)
  ...
  DO j=1,nx
     DO k=1,nsig
        DO i=1,nlat
           ii=ii+1
           bc1(j,i,k)=recvbuf(ii)*weight_bc1
           oc1(j,i,k)=recvbuf(ii)*weight_oc1
           p25(j,i,k)=recvbuf(ii)*weight_p25
        ENDDO
     ENDDO
  ENDDO

!leave BC2, OC2 for future
