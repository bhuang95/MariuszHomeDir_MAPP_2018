PROGRAM cal_gocart_aod

!Mariusz Pagowski Oct, 2016
  USE kinds,ONLY: r_kind,i_kind,r_single
  USE constants, ONLY : max_varname_length,ten,one,one_tenth,h300,r100,rd_over_cp_mass, &
...
!assume hourly intervals
  ...
  DO j=1,nlon
     DO k=1,nsig
        DO i=1,nlat
           pres_r(j,i,k)=1.e-3_r_kind*(eta(k)*(ps(j,i)-ptop)+ptop)
           tsen_r(j,i,k)=(tsen(j,i,k)+h300)*(pres_r(j,i,k)/r100)**rd_over_cp_mass
        ENDDO
     ENDDO
  ENDDO
...
!  PRINT *,MINVAL(sulf),MAXVAL(sulf)
!  PRINT *,MINVAL(bc1),MAXVAL(bc1),MINVAL(bc2),MAXVAL(bc2)
!  PRINT *,MINVAL(oc1),MAXVAL(oc1),MINVAL(oc2),MAXVAL(oc2)
!  PRINT *,MINVAL(dust1),MAXVAL(dust1),MINVAL(dust2),MAXVAL(dust2)
!  PRINT *,MINVAL(dust3),MAXVAL(dust3),MINVAL(dust4),MAXVAL(dust4)
!  PRINT *,MINVAL(dust5),MAXVAL(dust5),MINVAL(p25),MAXVAL(p25)
!  PRINT *,MINVAL(seas1),MAXVAL(seas1),MINVAL(seas2),MAXVAL(seas2)
!  PRINT *,MINVAL(seas3),MAXVAL(seas3),MINVAL(seas4),MAXVAL(seas4)
...
!needs to be specific humidity
        ...
        DO ii=1,nchanl
           total_aod(j,i,ii,it)=SUM(layer_od(:,ii))
        ENDDO
  ...
  PRINT *,MINLOC(total_aod(:,:,4,ntime)),MAXLOC(total_aod(:,:,4,ntime))
  
  maxlocs=MAXLOC(total_aod(:,:,4,ntime))

  DEALLOCATE(total_aod,aero)

END PROGRAM cal_gocart_aod
