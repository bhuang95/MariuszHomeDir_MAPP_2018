...
!  added 40% reduction of so2 - changed back to 100%
!  changed to new routines which accept varibale horizontal lenghtscales
...
!recursive filter parameters
  INTEGER, PARAMETER :: num_passes=16
  REAL, PARAMETER :: l_gaspari_log=10.,factor_so2=1.
  REAL, PARAMETER :: stdev_log=0.25 ! 0.3 test_12! .9 test_11 !.5 test_10 
  REAL :: rf_scale,rf_alpha,rf_scale_factor
  ...
  REAL, DIMENSION(:,:), ALLOCATABLE :: lscale
  ...
  CHARACTER(len=varname_length), DIMENSION(naero) :: aeronames=(/&
       ...
       &"E_SO4I" ,"E_SO4J" ,"E_NO3I","E_NO3J","E_SO2"/)

  CHARACTER(len=(varname_length+1)*naero) :: aerolist
  logical :: logaero=.false.
  ...
  INTEGER :: nx,ny
  REAL :: sum_cv,sum_cv2,mean_cv,rms_cv,std_dev_cv
  ...
  aerolist=''
  DO i=1,naero
     aerolist=TRIM(aerolist)//'.'//TRIM(aeronames(i))
  ENDDO
...
1000 CONTINUE
  ...
  ALLOCATE(data2(nx,ny,1,1),lscale(nx,ny))

  rf_scale=l_gaspari_log*SQRT(0.3)/4.

  DO i=1,nx
     DO j=1,ny
        lscale(i,j)=1./(rf_scale*2.)
!        lscale(i,j)=1./(rf_scale*2.+i/10.-j/10.)
     ENDDO
  ENDDO

  data2=0.
  data2(nx/2,ny/2,1,1)=1.

  CALL smoothx(nx-1,ny-1,data2,lscale,ndeg,rbe,nta,swidth,rtable)

  WRITE(501,*)nx,ny
  DO j=1,ny
     WRITE(501,'(80f10.3)')data2(:,j,1,1)
  ENDDO

  CALL smoothy(nx-1,ny-1,data2,lscale,ndeg,rbe,nta,swidth,rtable)

  WRITE(502,*)nx,ny
  DO j=1,ny
     WRITE(502,'(80f10.3)')data2(:,j,1,1)*rsamp*lscale(:,j)
!stdev is rescaled using rsamp and lscale     
  ENDDO

  data2=0.
  data2(nx/2,ny/2,1,1)=1.

  CALL da_perform_2drf(nx, ny, num_passes, rf_scale, data2(:,:,1,1))
  
  CALL da_calculate_rf_factors(rf_scale,rf_alpha, &
       rf_scale_factor, num_passes,1)

  WRITE(503,*)nx,ny
  DO j=1,ny
     WRITE(503,'(80f10.3)')data2(:,j,1,1)*rf_scale_factor
  ENDDO
...
! GET THE DIMS FOR INPUT AND OUTPUT FROM THE SHAPE
     ...
     IF     (itype .EQ. 2) THEN          ! character
        ...
        if (trim(cval) == "E_PM25I" .or. trim(cval) == "E_PM25J") then
           ...
           logaero=.true.
        else if (trim(cval) == "E_SO4I" .or. &
             &trim(cval) == "E_SO4J" ) then
           seed_newvalue=seed_value+350
           logaero=.true.
        else if (trim(cval) == "E_NO3I" .or. &
             &trim(cval) == "E_NO3J") then
           seed_newvalue=seed_value+450
           logaero=.true.
        ...
        if (logaero) then
           ...
           IF (generate_random_vector) THEN
              ...
              DO ii=1,nx
                 DO jj=1,ny
                    CALL da_gauss_noise(rcv(ii,jj))
                    sum_cv = sum_cv + rcv(ii,jj)
                    sum_cv2 = sum_cv2 + rcv(ii,jj) * rcv(ii,jj)
                 ENDDO
              ENDDO
              ...
              WRITE(unit=stdout,fmt='(3f15.5)'),mean_cv,rms_cv,std_dev_cv
              

!this two below are equivalent
!              rf_scale=l_gaspari_log*0.388/SQRT(8.)

              rf_scale=l_gaspari_log*sqrt(0.3)/4.
 
!check on gaussian scale             
!              rcv=0.
!              rcv(nx/2,ny/2)=1.

              CALL da_perform_2drf(nx, ny, num_passes, rf_scale, rcv)
...
!              WRITE(503,*)nx,ny
!              DO j=1,ny
!                 WRITE(503,'(80f10.3)')rcv(:,j)*rf_scale_factor
!              ENDDO
...
!           PRINT *,TRIM(cval),'IN'
           ...
           IF (TRIM(cval) == "E_SO2")  THEN
              DO jj=1,dims_out(4)
                 data2(:,:,:,jj) = DATA(:,:,:,jj)*factor_so2
              ENDDO
           ELSE
              DO jj=1,dims_out(4)
                 data2(:,:,1,jj) = DATA(:,:,1,jj)*&
                      &EXP(rcv(:,:)*rf_scale_factor*stdev_log)
                 data2(:,:,2:,jj)=DATA(:,:,2:,jj)
              ENDDO
           ENDIF
