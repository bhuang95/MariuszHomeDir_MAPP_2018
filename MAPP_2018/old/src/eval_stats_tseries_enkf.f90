...
!to remove some ridiculous fire values set model_fire_threshold=100.
...
!          &gridiobs_s(i),gridjobs_s(i)
     ...
     IF (l < 0) THEN
        ...
        IF (date > 0.) THEN
           ...
           date=(INT(date)/100-1)*24+MOD(INT(date),100)+deltat
           
           IF (MOD(INT(date),6)==0) &
                &WRITE(555,'(f7.2,i10)')date-deltat,0

           PRINT *,'Writing out for date ', date
           WRITE(outunit,'(f7.2,i10,8e15.7)')date,nobsvalid_s,&
                &bias_s,patrmse_s,corr_s,rmse_s,stdevobs_s,stdevfcst_s,&
                &obsave_s,fcstave_s
        ENDIF
