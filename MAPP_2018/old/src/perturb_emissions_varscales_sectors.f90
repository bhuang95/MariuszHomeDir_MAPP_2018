PROGRAM perturb_emissions_varscales_sectors
...
!recursive filter parameters
  REAL, PARAMETER :: factor_so2=.6,factor_ec=0.7
  ...
  INTEGER :: ndeg,nta
  REAL :: swidth,rsamp,factor
  REAL, DIMENSION(:,:), ALLOCATABLE :: rtable
  REAL, DIMENSION(:,:,:), ALLOCATABLE :: lscale,rf_scale,rcv_t
  ...
  CHARACTER cval*50,cval_lscale*50
  ...
  INTEGER             :: iidm,inatt,iitype,k,kk
...
!ncep scale 2*wrfvar
           ...
           rf_scale=1./(lscale*SQRT(0.3)/2.)
           
           DO kk=1,nt

              rcv_t(:,:,kk)=rcv

              CALL smoothx(nx-1,ny-1,rcv_t(:,:,kk),rf_scale(:,:,kk),&
                   & ndeg,rbe,nta,swidth,rtable)
              
              CALL smoothy(nx-1,ny-1,rcv_t(:,:,kk),rf_scale(:,:,kk),&
                   &ndeg,rbe,nta,swidth,rtable)
              
           ENDDO
        
           rcv_t=rcv_t*rsamp/rf_scale
...
!           PRINT *,TRIM(cval),'OUT'
  ...
  DEALLOCATE(lscale,rf_scale,rcv,rcv_t)
...
END PROGRAM perturb_emissions_varscales_sectors
