...
! uses WPS modules for map projections and gsi input files
  ...
  INTEGER, PARAMETER :: icut=2,jcut=2,fcst_interval=3
  
  REAL :: latc,lonc,truelat_1,truelat_2,stdlon,dx,dy

  CHARACTER(len=250) :: prefix,mapfile
  CHARACTER(len=12)species,spec
  CHARACTER(len=250) :: suffix='.bin'
  CHARACTER(len=250) :: obsfile,geoptfile,outfile,fcstdir

  INTEGER ::  nxin,nyin,nf,nx,ny,nz,ndays,nfcst,nfcst1
  ...
  CHARACTER(len=250), ALLOCATABLE, DIMENSION(:) :: fnames
...
!  Read all of the file names and store them.
  ...
  DO k=1,nf
     ...
     year=fnames(k)(i+2:i+5)
     month=fnames(k)(i+6:i+7)
     day=fnames(k)(i+8:i+9)
     hour=fnames(k)(i+10:i+11)
  ...
  julmax=jdate(iyearinit(nf),imonthinit(nf),idayinit(nf))+2
  ...
  DO WHILE(.TRUE.)
     ...
     IF (julobs >= julmin .AND. julobs <= julmax .AND. &
        ...
        julstamp(l)=REAL(julobs-julmin+1)*1e2+ihour
...
100 CONTINUE
  ...
  DO k=1,nf
     ...
     DO i=0,nfcst1
        ...
        ihour=i*fcst_interval+ihourinit(k)
        iday=ihour/24
        ihour=MOD(ihour,24)
...
END PROGRAM interp_fcst2obs_rt
