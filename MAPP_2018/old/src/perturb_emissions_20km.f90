...
!added scales variable for 20km
!also added factors for ec and so2
...
!recursive filter parameters
  INTEGER, PARAMETER :: num_passes=4
  REAL, PARAMETER :: factor_so2=0.6,factor_ec=0.7
  REAL, PARAMETER :: stdev_log=0.25 ! 0.3 test_12! .9 test_11 !.5 test_10 
  REAL :: rf_scale,rf_alpha,rf_scale_factor,factor,l_gaspari_log
...
!              jj=ny
!              do ii=1,nx
!                 write(501,*)ii,rcv(ii,jj)*rf_scale_factor
!              enddo
...
!           PRINT *,TRIM(cval),'IN'
           ...
           DO jj=1,dims_out(4)
              data2(:,:,1,jj) = DATA(:,:,1,jj)*&
                   &EXP(rcv(:,:)*rf_scale_factor*stdev_log)*factor
