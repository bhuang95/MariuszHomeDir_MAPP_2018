PROGRAM interp_fcst2obs_enkf_spec

!Mariusz Pagowski, CIRA, May. 2011

! program to interpolate forcasta to sfc obs
...
!factor is because for OC,EC, and NO3 observations were multiplied by 100 
  ...
  INTEGER ::  nxin,nyin,nf,nx,ny,nz,ndays,nfcst,nfcst1,ihourmin=999
  ...
  CALL getarg(3,tzones) 
!whether to match forecasts in timezones or total for the whole domain
  CALL getarg(4,obsfile)
  CALL getarg(5,fcstdir)
  call getarg(6,geoptfile)
  CALL getarg(7,outfile)

  IF (species=='EC' .OR. species=='OC' .OR. species=='NO3') THEN
     factor=100.
  ELSE
     factor=1.
  ENDIF
...
!  Read all of the file names and store them.
  ...
  DO WHILE(.TRUE.)
     ...
     IF (julobs >= julmin .AND. julobs <= julmax .AND. &
          &obs(l) >= 0. .AND. obs(l) < concmax*factor ) THEN
...
100 CONTINUE
  ...
  DO k=1,nf
     ...
     jul=(jdate(iyearinit(k),imonthinit(k),idayinit(k))-julmin+1)*1e2&
          &+ihourinit(k)
     
     ihourmin=MIN(ihourmin,ihourinit(k))
     
!     WRITE(outunit,*)iyearinit(k),imonthinit(k),idayinit(k),ihourinit(k)

     IF (tzones=='T') THEN
        WRITE(outunit,FMT=101)NINT(unknown),&
             &REAL(jul),unknown,&
             &unknown,unknown,NINT(unknown),unknown,unknown,&
             &unknown,unknown,NINT(unknown)

     ELSEIF (ihourinit(k)==ihourmin) THEN
        WRITE(outunit,FMT=101)NINT(unknown),&
             &REAL(jul),unknown,&
             &unknown,unknown,NINT(unknown),unknown,unknown,&
             &unknown,unknown,NINT(unknown)
...
102 FORMAT(i7,2(f10.3,f10.1),i5,4f10.4,a10)

END PROGRAM interp_fcst2obs_enkf_spec
