PROGRAM eval_stats_tseries_dots
...
!will evaluate stations where height is unknown=-9999.
  ...
  WRITE(outunit,'(a)')'# site_code,  lat,  lon, site_char, bias, prmse, corr  '
...
!             &gridiobs_s(i),gridjobs_s(i),code_new
     ...
     IF (nobsvalid_s > istatsmin .AND. loghdiff) THEN
        ...
        WRITE(outunit,FMT=103)code_s(1),delim,lat_s(1),delim,lon_s(1),&
             &delim,site_s(1),delim,&
             &bias_s,delim,patrmse_s,delim,corr_s,delim,stdevobs_s,&
             &delim,stdevfcst_s,delim,obsave_s,delim,fcstave_s
...
103 FORMAT(i10,2(a,f10.3),a,i5,7(a,f10.4))

END PROGRAM eval_stats_tseries_dots
