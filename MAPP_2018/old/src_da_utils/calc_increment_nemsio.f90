...
!calculate difference between analysis and fcst
!script in /home/Mariusz.Pagowski/codes/fv3/util_scripts
!may recode to get variables not records at some time
  
  USE nemsio_module
  USE netcdf_io, ONLY : writenetcdfdata3
...
!
  CHARACTER(500) :: filename_in_1,filename_in_2,filename_nc_out
  INTEGER :: unit_out=101
  INTEGER :: iargc
  REAL, ALLOCATABLE :: vcoord(:,:,:)
  INTEGER :: k
  REAL, DIMENSION(:), ALLOCATABLE :: pk,bk
  REAL, DIMENSION(:,:,:), ALLOCATABLE :: aero_increment
  LOGICAL :: skip
...
!--- meta data info
  ...
  diff_time=nfhour_1-nfhour_2
  PRINT *,'Time difference between analysis and forecast  ',diff_time
...
!from jsw comments in regrid-nemsio  
  ...
  ALLOCATE(aero_increment(im,jm,lm))

  vlevtyp='mid layer'

  DO k=1,ntracers_gocart
     vname=TRIM(tracer_names_gocart(k))

     CALL nemsio_searchrecv(gfile_1,jrec,vname,'mid layer',1,iret)
     IF (iret /= 0) THEN 
        PRINT *,'Variable not in file ',gfile_1,vname
        STOP
     ENDIF

     CALL nemsio_searchrecv(gfile_2,jrec,vname,'mid layer',1,iret)
     IF (iret /= 0) THEN 
        PRINT *,'Variable not in file ',gfile_2,vname
        STOP
     ENDIF

     DO l=1,lm
        CALL nemsio_readrecv(gfile_1,vname,'mid layer',l,tmp_1,iret=iret)
!@mzp
!        PRINT *,TRIM(filename_in_1),TRIM(vname),l,MINVAL(tmp_1),MAXVAL(tmp_1)
        IF (iret /= 0) THEN 
           PRINT *,'Variable level not in file ',gfile_1,vname,l
           STOP
        ENDIF

        CALL nemsio_readrecv(gfile_2,vname,'mid layer',l,tmp_2,iret=iret)
!        PRINT *,TRIM(filename_in_2),TRIM(vname),l,MINVAL(tmp_2),MAXVAL(tmp_2)
        IF (iret /= 0) THEN 
           PRINT *,'Variable level not in file ',gfile_2,vname,l
           STOP
        ENDIF
        ...
        DO j=jm,1,-1
           DO i=1,im
              ii=ii+1
              aero_increment(i,j,lm-l+1)=tmp_2(ii)-tmp_1(ii)
           ENDDO
        ENDDO
     ENDDO
     
     varname=TRIM(vname)//'_inc'
     
     PRINT *,'Outputtine ncdf ',varname
     CALL writenetcdfdata3(filename_nc_out,aero_increment,varname,im,jm,lm)
     
  ENDDO
  
  IF (ALLOCATED(tmp_1)) DEALLOCATE(tmp_1)
  IF (ALLOCATED(tmp_2)) DEALLOCATE(tmp_2)
  IF (ALLOCATED(aero_increment)) DEALLOCATE(aero_increment)
