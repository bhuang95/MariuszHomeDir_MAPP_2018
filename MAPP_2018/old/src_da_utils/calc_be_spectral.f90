...
!  filenamein = "sfg_"//datestring//"_fhr06s_mem"//TRIM(charnanal)//"_t254"
  ...
  DO itrac=1,ntrac
     ...
     CALL getsigdata(sigheadi,sigdatai,psig,psg,pressl,zsg,ak,bk,&
          &nlons,nlats,nlevs,ntrunc,itrac,mpi_comm_world)
     IF (itrac == ntrac) THEN
...
! calculate length scale (eqn 5 in pannekoucke, berre and desroziers)
     ...
     IF (nproc == 0) THEN
        ...
        IF (itrac == 1) THEN
           ...
           WRITE(9) INT(nlons),INT(nlats),INT(nlevs),INT(nanals)
           WRITE(9) REAL(ASIN(gaulats))
           WRITE(9) REAL(pressl)
           ALLOCATE(pressi(nlons,nlats,nlevs+1))
           DO k=1,nlevs+1
              pressi(:,:,k) = -LOG(ak(k)/psg+bk(k))
           ENDDO
           WRITE(9) REAL(pressi)
           DEALLOCATE(pressi)
           CLOSE(9)
        ...
        filenameout='be_tracer_'//ctrac//'.bin'
        OPEN(9,file=filenameout,form='unformatted')
        WRITE(9) INT(nlons),INT(nlats),INT(nlevs),INT(nanals)
        WRITE(9) REAL(psigvar)
        WRITE(9) REAL(lengthscalex)
        WRITE(9) REAL(lengthscaley)
        WRITE(9) REAL(vlengthscale)
        CLOSE(9)
        
        filenameout='be_tracer_'//ctrac//'.txt'

        OPEN(19,form='formatted',file=filenameout)
        DO i=1,nlats
           DO k=1,nlevs
              WRITE(19,'(i5,f8.3,e15.5,2f15.3,f10.4)')&
                   &k,ASIN(gaulats(i))*rad2deg,&
