...
! nproc is process number, numproc is total number of processes.
 ...
 IF (nens /= numproc) THEN
  ...
  DO ivar=1,nvars3d
     ...
     IF (.NOT. ALLOCATED(var)) ALLOCATE(var(nxloc,nyloc,nzloc,1))
     IF (.NOT. ALLOCATED(vardata)) ALLOCATE(vardata(nxloc,nyloc,nzloc,1))
     
     DO itime=1,nt

        IF (nproc == 0) PRINT *,TRIM(varstringname), itime
        
        var=0.
        
        PRINT *,'@@1'

        CALL readnetcdfdata4_tslice(filenamein,vardata,varstringname,&
             &nxloc,nyloc,nzloc,itime)

        PRINT *,'@@2'
        PRINT *,nt,nproc,MINVAL(vardata),MAXVAL(vardata)
        

        CALL mpi_allreduce(vardata,var,SIZE(var),mpi_real,mpi_sum,mpi_comm_world,iret)

        PRINT *,nt,nproc,MINVAL(var),MAXVAL(var)
