PROGRAM add_aero_bckg
  ...
  INTEGER(i_kind) :: inerr,nlat,nlon,nsig,nsigstat,mlat,iret
...
110 CONTINUE
  ...
  ALLOCATE(allbes(nlon,nlat-2,nsig-1),allbes_y(nlon,nlat-2,nsig-1),allbes_zonal(nlat,nsig))
  ...
  DO i=1,ntracers_gocart
     ...
     IF (nlon_aero /= nlon .OR. mlat+2 /= nlat .OR. nsigstat+1 /= nsig) THEN
     ...
     allbes_zonal(:,nsig)=allbes_zonal(:,nsig-1)

     WRITE(outerr)allbes_zonal
