...
! Declare public and private
  ...
  INTEGER(i_kind), PARAMETER :: nchan_modis=20, ichan_modis_550=4
...
!    header_fix%nchan   = nchanl
    IF (nchanl /= nchan_modis) THEN
       PRINT *,'only works for 20-channel modis - stopping'
       STOP
    ENDIF
    header_fix%nchan   = 1
...
!    DO ich=1, header_fix%nchan
    ...
    DO ich=1, nchanl
       ...
       IF (iuse_tmp == 1) THEN
          ...
          IF (nuchan_tmp /= ichan_modis_550) THEN
             PRINT *,'only works for modis channel at 550nm - stopping'
             stop
          ENDIF

          header_chan(1)%freq     = freq_tmp
          header_chan(1)%polar    = polar_tmp
          header_chan(1)%wave     = wave_tmp
          header_chan(1)%varch    = varch_tmp
          header_chan(1)%iuse     = iuse_tmp
          header_chan(1)%nuchan   = nuchan_tmp
          header_chan(1)%iochan   = iochan_tmp
...
! Allocate arrays to hold data record
    ALLOCATE(data_tmp(ipchan_aod,nchan_modis))

    ALLOCATE( fix_tmp( ireal_aod ) )

! Read data record

    READ(ftin,IOSTAT=iflag,END=1000,ERR=1000) fix_tmp, data_tmp
...
!    PRINT *,'@@@2',data_fix%lat,data_fix%lon,data_fix%psfc,data_fix%obstime,&
!         &data_fix%solzen_ang,data_fix%solazm_ang

!    PRINT *,'@@@3',data_tmp(1:ipchan_aod,ichan_modis_550)

! Transfer data record to output structure
!    DO ich=1,header_fix%nchan
    DO ich=1,nchan_modis
       IF (ich == ichan_modis_550) THEN
          data_chan(1)%aodobs =data_tmp(1,ich)
          data_chan(1)%omgaod =data_tmp(2,ich)
          data_chan(1)%errinv=data_tmp(3,ich)
          data_chan(1)%qcmark=data_tmp(4,ich)
       ENDIF
    ENDDO

    PRINT *,'@@@4',data_chan(1)%aodobs,data_chan(1)%omgaod,data_chan(1)%errinv,data_chan(1)%qcmark
