...
!   lowcase each letter if it is lowecase
  ...
  INTEGER(int32) FUNCTION getindex(varnames,usrname)
    IMPLICIT NONE
  ...
  SUBROUTINE indexx_r(n,arr,indx)
    INTEGER(int32) n,indx(n),M,NSTACK
    REAL arr(n)
    PARAMETER (M=7,NSTACK=50)
    INTEGER(int32) i,indxt,ir,itemp,j,jstack,k,l,istack(NSTACK)
    REAL a
    DO j=1,n
       indx(j)=j
    ENDDO
...
5      indx(l)=indx(j)
  ...
  SUBROUTINE rank(n,indx,irank)
    INTEGER(int32) n,indx(n),irank(n)
    INTEGER(int32) j
    DO j=1,n
       irank(indx(j))=j
    ENDDO
