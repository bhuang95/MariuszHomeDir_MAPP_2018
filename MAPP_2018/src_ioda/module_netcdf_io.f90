...
CONTAINS
  ...
  SUBROUTINE netcdf_read_strings(fnamein,varname,vardata)
    ...
    INTEGER(int32)                         :: ncid,varid,status,ndims
    INTEGER(int32)                         :: dimids(1:ndims_max),dims(1:ndims_max)
    INTEGER(int32) :: i,n
  ...
  SUBROUTINE netcdf_read(fnamein,varnamein,vardata,dim2reverse)
    ...
    INTEGER(int32), INTENT(in), OPTIONAL               :: dim2reverse

    INTEGER(int32)                         :: ncid,varid,status,ndims
    INTEGER(int32)                         :: dimids(1:ndims_max),dims(1:ndims_max)
    REAL(SINGLE), ALLOCATABLE       :: tmp(:,:,:,:,:)
    
    INTEGER(int32) :: i,n,attnum
...
!always convert all vars to real     
  ...
  SUBROUTINE netcdf_write_fv3_aod(fnameout,aodsfc,aodlayers,channels)
    ...
    INTEGER(int32)                         :: mcid,aodsfcid,aodlayersid,channelsid,status
    INTEGER(int32)                         :: dimids(1:ndims_max),dims(1:ndims_max)
...
!writes out only channel 4 for viirs
    ...
    INTEGER(int32)                         :: mcid,varid,hofxid,status
    INTEGER(int32)                         :: dimids(1:ndims_max),&
         &dims(1:ndims_max)

    CHARACTER(len=max_varname_length) :: varname

    INTEGER(int32) :: i,ndims
  ...
  SUBROUTINE netcdf_write_generic_pll(init,fnameout,lon,lat,plev,&
    ...
    INTEGER(int32), SAVE :: nlon, nlat, nplev, nt
    INTEGER(int32), SAVE :: dimids(ndims)
  ...
  CONTAINS
    ...
    SUBROUTINE netcdf_init_filepll(fnameout,lon,lat,plev,datestrings)
      ...
      INTEGER(int32) :: mcid,lonvar_id,lon_id,lat_id,lev_id,tm_id,&
           &timestr_id,&
...
!         tmp=RESHAPE(vardata,shape=[nlon,nlat,nplev,nt],order=[2,1,3,4])
  ...
  SUBROUTINE netcdf_write_fv3aodll(init,fnameout,lon,lat,channels,&
    ...
    INTEGER(int32), PARAMETER :: ndims=4
    INTEGER(int32), SAVE :: nlon, nlat, nchannels, nt
    INTEGER(int32), SAVE :: dimids(ndims)
    REAL :: attvaluer,attvalueranger(2),fillvalue=1.e+15    
    INTEGER(int32), PARAMETER :: deflate_level=2
  ...
  SUBROUTINE netcdf_write_generic_ll(init,fnameout,lon,lat,lev,&
    ...
    INTEGER(int32), INTENT(in), OPTIONAL :: lev(:)
    CHARACTER(len=*), DIMENSION(:), INTENT(in), OPTIONAL :: datestrings
    CHARACTER(len=*), INTENT(in), OPTIONAL :: varname
    ...
    INTEGER(int32) :: attvaluei,attvaluerangei(2)
    REAL :: attvaluer,attvalueranger(2),fillvalue=1.e+15


    INTEGER(int32), SAVE :: nlon, nlat, nlev, nt
    INTEGER(int32), SAVE :: dimids2d(ndims-1),dimids3d(ndims)
    ...
    ELSE
       ...
       CALL netcdf_writevar_filell(fnameout,varname,vardata,&
            &units,validrange)
    ENDIF
...
!            tmp=RESHAPE(vardata,shape=[nlon,nlat,nlev,nt],order=[2,1,3,4])
      ...
      IF (SIZE(vardata,2)==1) THEN
         status = nf90_def_var(mcid,varname,NF90_REAL,&
  ...
  SUBROUTINE netcdf_write_fv3_concentration(fnameout,var,varname)
    ...
    INTEGER(int32), PARAMETER              :: ndims_max=4
    INTEGER(int32)                         :: mcid,tid,varid,status
    INTEGER(int32)                         :: dimids(1:ndims_max),dims(1:ndims_max)
    CHARACTER(len=max_varname_length), PARAMETER    :: dimstring(1:ndims_max)=&
