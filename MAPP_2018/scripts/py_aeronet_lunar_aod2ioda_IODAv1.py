    ...
    def calc_new_aod_values(self):
        ...
        def _tspack_aod_interp(row, new_wv=[440., 470., 550., 670., 870., 1240.]):
            ...
            if len(df_aod_nu_sorted) < 2:
                ...
                x, y, yp, sigma = pytspack.tspsi(np.log(df_aod_nu_sorted.wv.values.astype('float64')), np.log(df_aod_nu_sorted.aod.values.astype('float64')))
                yi = pytspack.hval(np.log(new_wv.astype('float64')), x, y, yp, sigma)
                return np.exp(yi)
...
if __name__ == '__main__':
    ...
    aod_new_chan = None #np.array([9], dtype=np.intc)
    if aod_new_wav is not None: 
    ...
    f3_tmp = add_data(dates=dates, product=aodqc, lunar_merge=lunar, interp_to_aod_values=aod_new_wav)
    ...
    var_mdata['wavelength'] = writer.FillNcVector(aod_wav_m, 'float')
    ...
    #outdata[varDict['aerosol_optical_depth_9']['qcKey']] = outdata[varDict['aerosol_optical_depth_4']['qcKey']]
    #print(type(outdata[varDict['aerosol_optical_depth_4']['qcKey']]))
    #np.savetxt('preqc_500nm.txt', outdata[varDict['aerosol_optical_depth_4']['qcKey']])
    #np.savetxt('preqc_550nm.txt', outdata[varDict['aerosol_optical_depth_9']['qcKey']])

    # Define global atrributes
    print('Define global atrributes')
    AttrData = {'observation_type': aodqc,
