...
#sbatch --export=ALL sbatch_convert_c48_c24.sh
...
OUTDIR_C24=${MAINDIR}/C24/RESTART

/bin/cp ${INDIR_C96}/gfs_aero_c96/akbk.nc ${OUTDIR_C24}/gfs_aero_c24
...
ln -sf ${OUTDIR_C24} outputs

/bin/cp ${JEDIYAML}/gfs_c96_to_c24_template.yaml .
...
do
	...
	if [[ ! -r ${OUTDIR_C24}/gfs_aero_c24/${charnanal} ]]
	then
	    mkdir -p ${OUTDIR_C24}/gfs_aero_c24/${charnanal}
	fi
	
	sed -e "s/member/${charnanal}/g" -e "s/analdate/${analdate}/g" gfs_c96_to_c24_template.yaml > gfs_c96_to_c24.yaml

	exit
	
	export jobtype=gfs_c96_to_c24
