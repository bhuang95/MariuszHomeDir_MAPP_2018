import sys,os
#below figure how to create a module
sys.path.append('/scratch1/BMC/gsd-fv3-dev/MAPP_2018/pagowski/python/lib')
...
sa_path1="%s/wrk-%s-%s/%s-%s/met_tool_wrapper/stat_analysis/" %(expdir, modelname, obsname1, modelvar, obsvar) 
...
while (cdate<edate):
    ...
    filename1="%s/%s/%s/%s_%s_%s.stat" %(sa_path1,modelname,cdate,area,obsvar,linetype)
    f1=open(filename1,'r')
    nlev=-1
...
for nlv in np.arange(nlev):
    ...
    if (fsave):
        ...
        fig.savefig('./TimeSeries_P%s_%s.%s_%s_%s_%s.%s_%s.png' %(str(plev[nlv]),area,obsvar,obsname1,modelname,obsname,sdate,cdate), format='png')
...
if (fsave):
    ...
    fig.savefig('./Profile_%s.%s_%s_%s_%s.%s_%s.png' %(area,obsvar,obsname1,modelname,obsname,sdate,cdate), format='png')
